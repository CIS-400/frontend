...
class TradeOfferStagingArea extends PIXI.Container implements Updatable {
  ...
  public update() {
    for (let i = 0; i < SETTLERS.NUM_RESOURCE_TYPES; i++) {
      this.offerText[i].text = this.offer.get(i as SETTLERS.Resource)
      this.requestText[i].text = this.request.get(i as SETTLERS.Resource)
    }
  }
